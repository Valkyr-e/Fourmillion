...
func find_nearest_enemy() :
	...
	for i in range (1,len(enemies)):
		...
		#verification avec un raycast pour voir si accessible en ligne droite
		var space_state = get_world_2d().direct_space_state
		var query = PhysicsRayQueryParameters2D.create(global_position, pretender.global_position,8, [])
		var result = space_state.intersect_ray(query)
		if pretender_distance_squared < shortest_distance_squared  and result == {}:
			nearest_enemy = pretender
			shortest_distance_squared = pretender_distance_squared
	var space_state = get_world_2d().direct_space_state
	var query = PhysicsRayQueryParameters2D.create(global_position, nearest_enemy.global_position,8, [])
	var result = space_state.intersect_ray(query)
	if result != {}:
		nearest_enemy = null
